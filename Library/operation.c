...
/*------ private variable --------------------------*/
bit enable_sensor_delayEN = 0;		// ÑÓ³ÙÊ¹ÄÜ´«¸ÐÆ÷µÄÊ¹ÄÜ£¬²»ÄÜ¼´Ê±Ê¹ÄÜ´«¸ÐÆ÷£¬ÐèÒª¹ýÒ»¶ÎÊ±¼ä		
...
tWord ADC_check_saved_result = 0;		//×÷ÎªAD¼ì²âÖµµÄ´æ´¢Öµ£¬¼´ÉÏÒ»´Î²¥±¨µÄÖµ¡£
...
void Reset_after_stolen_alarming(void)
	...
	if(EN_host_stolen_alarming == 1)
		...
		if(++Stolen_alarm_reset_count > 3)
			...
			// ç”µæœºé”æ­»å…³é—­ï¼Œå¹¶ä¸”æŽ§åˆ¶å™¨ç”µæºæ‰“å¼€
			Generator_lock = 0;
			Externalmotor = 1;
			}
		}
...
void Detect_open_action(void)
	...
	if(((key_rotate == 1)||(Emergency_open_G == 1))&&(Open_action_flag == 0)&&(ID_certificated_flag == 1)&&(never_alarm == 0))		
		...
		if(Just_power_up == 1)		//åˆ¤æ–­
		{
			if(ADC_check_result < 0x236)
			{
				Battery_flag = 0;		//è¡¨ç¤ºç”µç“¶ä¸º48Vç”µç“¶
			}
			else
			{
				Battery_flag = 1;		//è¡¨ç¤ºç”µç“¶ä¸º60Vç”µç“¶
			}		
		}
		Just_power_up = 0;
